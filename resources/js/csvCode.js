...
var leftBox;
var rightBox;
var repetitions = 4; //number of repetitions
...
function saveToCsv(){
	var lineArray = [];
	let csvContent = "data:text/csv;charset=utf-8,width(px);distance(px);mt(ms);"+inputDeviceValue+"\n";
...
function onStartButtonClicked(){
	if(completedCombinations.length>=numberOfConditions){
		...
		inputDevice.style.display="none";
...
function inputFilled(event){
	if(event.target.value.length!=0){
		startButton.classList.remove("disabledButton");
		inputDeviceValue=event.target.value;
	}
	else{
...
function init(){
	...
	rightBox.addEventListener("click", onRightBoxClicked);
	leftBox.addEventListener("click",onLeftBoxClicked);
	startButton = document.querySelector("#start-button");
	startButton.addEventListener("click",onStartButtonClicked);
	startButton.classList.add("disabledButton");
	inputDevice = document.querySelector("#task-input");
	inputDevice.addEventListener("input",inputFilled);
}
*/
